...
# img = string containing filename/path if outside current directory and file extension
# int samplePathLength that dictates the length of each search path sent out to detect
# 	the width of a vein contour
# int div factor. Divides each contour into (length contour)/div factor to determine
# 	number of search paths per contour
# int min contour area dictates the pixel area threshold for a contour to be analyzed
# int windowLength determines how many consecutive contour width measurements are
# 	averaged over to determing a good landing strip for the needle
# boolean debugOut determines whether or not to output annotated/intermediate images

# the function returns a tuple (x,y) of the optimal location

# WARNING!!!: Values may need to be changed depending on imaging conditions such as 
# 	brightness, density of arm hair, orientation of arm in device. Contact Robert Shi
# 	at 1-800-GET-REKT for a free initial consultation on how screwed you are if you need
# 	to fix this mess.
def imageProcess(img, samplePathLength, divFactor, minContourArea, windowLength,debugOut):
	...
	if debugOut == False:
		return (tuple(optimalLocation[0]))
	# Write return here
...
print(imageProcess('CLAHE_ENHANCED_veinImage_62gs6.png',600,5,50000,10,True))
