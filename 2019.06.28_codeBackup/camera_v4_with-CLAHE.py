...
def printerMove(ser,x,y,z,f):
    ...
    if abs(x) > 100 or abs(y) > 100 or abs(z) > 10:
        ...
        #remember the b
        ser.write(strToBin(cmdRelativelative))
        printerCheckOk(ser)
        sleep(0.2)
        ser.write(strToBin(cmdMove))
        printerCheckOk(ser)
        sleep(1/(max([x, y, z])*60*f*1.1))
        
#convert ot ascii bytes
def strToBin(command):
    return bytes(command, encoding = "ascii")

#check for an okay
def printerCheckOk(ser):
    if ser.is_open():
...
def mainLoop(camera, ser):
    ...
    printerCheckOk(ser)
    sleep(5)
    printerMove(ser, 0, 100, 0, 2500)
    printerMove(ser, 100, 0, 0, 2500)
    printerMove(ser, 0, -100, 0, 2500)
    printerMove(ser, -100, 0, 0, 2500)
    terminate(camera, ser)
